...
export default function Chat({ agent }: ChatProps) {
  ...
  const handleLogin = (apiKey: string, provider: 'groq' | 'anthropic' | 'gemini') => {
    setShowLogin(false);
    // Save the API key persistently
    agent.saveApiKey(apiKey, provider);
    const providerName = provider === 'groq' ? 'Groq' : provider === 'anthropic' ? 'Anthropic' : 'Gemini';
    addMessage({
      role: 'system',
      content: `${providerName} API key saved successfully. You can now start chatting with the assistant.`,
    });
  };
  ...
  const handleModelSelect = (model: string, provider: 'groq' | 'anthropic' | 'gemini') => {
    ...
    agent['configManager']?.setProvider(provider);
    // Clear the client to force re-initialization with the correct provider
    agent['client'] = null;
    agent['geminiClient'] = null;
    const providerName = provider === 'groq' ? 'Groq' : provider === 'anthropic' ? 'Anthropic' : 'Gemini';
    addMessage({
      role: 'system',
      content: `Switched to ${providerName} model: ${model}. Chat history has been cleared.`,
    });
  };
