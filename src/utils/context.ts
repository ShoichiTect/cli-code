...
export function generateProjectContext(
...
): {json: ProjectContextJson; markdown: string} {
	...
	try {
		...
		throw new Error(
			`Root directory does not exist or is not accessible: ${root}`,
		);
	}
...
function readPackageJson(
	...
	try {
		...
		return {
			...
			dependencies_count: pkg.dependencies
...
function buildMarkdown(ctx: ProjectContextJson): string {
	...
	if (ctx.package) {
		lines.push(
			`- Package: ${ctx.package.name || ''} ${
				ctx.package.version ? `v${ctx.package.version}` : ''
			}`.trim(),
		);
		...
		lines.push(
			`- Dev Dependencies: ${ctx.package.dev_dependencies_count || 0}`,
		);
	}
	lines.push('');
	lines.push('## Languages (by file count)');
	const langLines = ctx.summary.languages
		.slice(0, 12)
		.map(l =>
			l.extension === '(none)'
				? `- ${l.extension}: ${l.files}`
	...
	lines.push(
		...(langLines.length ? langLines : ['- (no code files detected)']),
	);
